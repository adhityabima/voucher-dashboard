...
public class VoucherController {
    ...
    @PostMapping
    public ResponseEntity<VoucherDto> createVoucher(@RequestBody VoucherDto body) {
        VoucherDto response = voucherService.createVoucher(body);
        return new ResponseEntity<>(response, HttpStatusCode.valueOf(201));
    }

    @PatchMapping("/{id}")
    public ResponseEntity<VoucherDto> updateVoucher(@Param("id") Long id, @RequestBody VoucherDto body) throws NotFoundException {
        VoucherDto response = voucherService.updateVoucher(id, body);
        return new ResponseEntity<>(response, HttpStatusCode.valueOf(200));
    }  

    @DeleteMapping("/{id}")
    public ResponseEntity<String> deleteVoucher(@Param("id") Long id) throws NotFoundException {
